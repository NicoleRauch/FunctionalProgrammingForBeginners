...
\onslide+<4->
\item \de{In Java nicht direkt unterstützt}

\onslide+<5->
\item \de{Kann trotzdem leicht verwirklicht werden:}
\end{itemize}
...
\begin{itemize}
\onslide+<2->
\item \de{\glqq{}alles, was den Ablauf eines Computerprogramms oder die Außenwelt verändert, ohne von einer Funktion zurückgegeben worden zu sein\grqq{}}
...
\item Exceptions
\item Logging
\item Abhängigkeit von (externen) Konfigurationen
\item Veränderungen des Zustands 
\item Nichtdeterminismus (z. B. durch die Verwendung eines Zufallszahlengenerators).
\end{itemize}

\onslide+<4->
\item \de{Manche Sprachen zeigen das Vorhandensein von Seiteneffekten sogar in der Typsignatur an}
...
\onslide+<2->
\de{In manchen funktionalen Sprachen schreiben wir:}\en{In real functional languages we write:}
\begin{lstlisting}{Haskell}
times x y = x * y
\end{lstlisting}
...
Java 8:
\begin{lstlisting}{Java}
Arrays.asList(1, 2, 3, 4).stream().filter(x -> x % 2 == 0).toArray();          // new Integer[]{2, 4}
\end{lstlisting}
